...
# samlify = { version = "0.1", default-features = false }
```
...
`samlify` is aimed at applications that need SAML SP/IdP flows without a C XML
...
the native `xmlsec` stack, while `samlify` keeps the SAML protocol path
...
```sh
cargo run -p samlify --example sso
```
...
The [crate-root docs](https://docs.rs/samlify/latest/samlify/) contain
doctested fragments for the typed `Saml` facade, including
[`Saml<Sp>::start_sso`](https://docs.rs/samlify/latest/samlify/struct.Saml.html#method.start_sso),
[`Saml<Sp>::finish_sso`](https://docs.rs/samlify/latest/samlify/struct.Saml.html#method.finish_sso),
[`Saml<Idp>::receive_sso`](https://docs.rs/samlify/latest/samlify/struct.Saml.html#method.receive_sso),
and [`Saml<Sp>::finish_slo`](https://docs.rs/samlify/latest/samlify/struct.Saml.html#method.finish_slo).
...
fragment](https://docs.rs/samlify/latest/samlify/#sp-initiated-sso) for the
compact API shape.
...
[Identity Provider flows](https://docs.rs/samlify/latest/samlify/#identity-provider-flows)
crate-root section.
...
fragment](https://docs.rs/samlify/latest/samlify/#single-logout) for the compact
shape.

### Metadata

Metadata trust is explicit. The rustdoc
[Metadata trust](https://docs.rs/samlify/latest/samlify/#metadata-trust)
...
[`SamlValidationContext`](https://docs.rs/samlify/latest/samlify/struct.SamlValidationContext.html).

### Advanced/raw compatibility

The low-level compatibility API remains available under `samlify::raw` for
...
Use visible docs.rs modules, crate-root re-exports, and `samlify::raw` before
...
- XML-Enc support is available, but software RSA key-transport decryption is
  ...
  [`XmlEncryptionPolicy`](https://docs.rs/samlify/latest/samlify/struct.XmlEncryptionPolicy.html).

## Security

`samlify` is pre-1.0 and has not had an external security audit. Review the
...
```sh
cargo fmt --all --check
cargo clippy -p samlify --all-targets -- -D warnings
cargo nextest run -p samlify
cargo test -p samlify --doc
RUSTDOCFLAGS="-D warnings -D missing_docs" cargo doc -p samlify --lib --all-features --no-deps
cargo test -p samlify --doc --no-default-features
cargo check -p samlify --no-default-features
```
