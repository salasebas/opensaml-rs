...
# samlet = { version = "0.1", default-features = false }
```
...
`samlet` is aimed at applications that need SAML SP/IdP flows without a C XML
...
the native `xmlsec` stack, while `samlet` keeps the SAML protocol path
...
```sh
cargo run -p samlet --example sso
```
...
The [crate-root docs](https://docs.rs/samlet/latest/samlet/) contain
doctested fragments for the typed `Saml` facade, including
[`Saml<Sp>::start_sso`](https://docs.rs/samlet/latest/samlet/struct.Saml.html#method.start_sso),
[`Saml<Sp>::finish_sso`](https://docs.rs/samlet/latest/samlet/struct.Saml.html#method.finish_sso),
[`Saml<Idp>::receive_sso`](https://docs.rs/samlet/latest/samlet/struct.Saml.html#method.receive_sso),
and [`Saml<Sp>::finish_slo`](https://docs.rs/samlet/latest/samlet/struct.Saml.html#method.finish_slo).
...
fragment](https://docs.rs/samlet/latest/samlet/#sp-initiated-sso) for the
compact API shape.
...
[Identity Provider flows](https://docs.rs/samlet/latest/samlet/#identity-provider-flows)
crate-root section.
...
fragment](https://docs.rs/samlet/latest/samlet/#single-logout) for the compact
shape.

### Metadata

Metadata trust is explicit. The rustdoc
[Metadata trust](https://docs.rs/samlet/latest/samlet/#metadata-trust)
...
[`SamlValidationContext`](https://docs.rs/samlet/latest/samlet/struct.SamlValidationContext.html).

### Advanced/raw compatibility

The low-level compatibility API remains available under `samlet::raw` for
...
Use visible docs.rs modules, crate-root re-exports, and `samlet::raw` before
...
- XML-Enc support is available, but software RSA key-transport decryption is
  ...
  [`XmlEncryptionPolicy`](https://docs.rs/samlet/latest/samlet/struct.XmlEncryptionPolicy.html).

## Security

`samlet` is pre-1.0 and has not had an external security audit. Review the
