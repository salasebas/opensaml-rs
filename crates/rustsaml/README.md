...
# rustsaml = { version = "0.1", default-features = false }
```
...
`rustsaml` is aimed at applications that need SAML SP/IdP flows without a C XML
...
the native `xmlsec` stack, while `rustsaml` keeps the SAML protocol path
...
```sh
cargo run -p rustsaml --example sso
```
...
The [crate-root docs](https://docs.rs/rustsaml/latest/rustsaml/) contain
doctested fragments for the typed `Saml` facade, including
[`Saml<Sp>::start_sso`](https://docs.rs/rustsaml/latest/rustsaml/struct.Saml.html#method.start_sso),
[`Saml<Sp>::finish_sso`](https://docs.rs/rustsaml/latest/rustsaml/struct.Saml.html#method.finish_sso),
[`Saml<Idp>::receive_sso`](https://docs.rs/rustsaml/latest/rustsaml/struct.Saml.html#method.receive_sso),
and [`Saml<Sp>::finish_slo`](https://docs.rs/rustsaml/latest/rustsaml/struct.Saml.html#method.finish_slo).
...
fragment](https://docs.rs/rustsaml/latest/rustsaml/#sp-initiated-sso) for the
compact API shape.
...
[Identity Provider flows](https://docs.rs/rustsaml/latest/rustsaml/#identity-provider-flows)
crate-root section.
...
fragment](https://docs.rs/rustsaml/latest/rustsaml/#single-logout) for the compact
shape.

### Metadata

Metadata trust is explicit. The rustdoc
[Metadata trust](https://docs.rs/rustsaml/latest/rustsaml/#metadata-trust)
...
[`SamlValidationContext`](https://docs.rs/rustsaml/latest/rustsaml/struct.SamlValidationContext.html).

### Advanced/raw compatibility

The low-level compatibility API remains available under `rustsaml::raw` for
...
Use visible docs.rs modules, crate-root re-exports, and `rustsaml::raw` before
...
`rustsaml` is pre-1.0 and has not had an external security audit. Review the
...
```sh
cargo fmt --all --check
cargo clippy -p rustsaml --all-targets -- -D warnings
cargo nextest run -p rustsaml
cargo test -p rustsaml --doc
RUSTDOCFLAGS="-D warnings -D missing_docs" cargo doc -p rustsaml --lib --all-features --no-deps
cargo test -p rustsaml --doc --no-default-features
cargo check -p rustsaml --no-default-features
```
